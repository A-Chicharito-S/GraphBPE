...
class MolGraph(object):
    ...
    def __init__(self, smiles):
        ...
        if self.mol is None:
            self.clusters = []  # since here we only care about the clustering
        else:
            self.mol_graph = self.build_mol_graph()
            self.clusters = self.find_clusters()
            # print('cluster created', self.clusters)
            self.clusters, self.atom_cls = self.pool_clusters()
            # print('cluster after pool clusters', self.clusters)
            self.mol_tree = self.tree_decomp()
            # print('cluster after tree decomp', self.clusters)
            self.order = self.label_tree()
            # print('cluster after label tree', self.clusters)
    ...
    def pool_clusters(self):
        ...
        for fsmiles, fatoms in fragments:
            if fsmiles not in MolGraph.FRAGMENTS:
                continue
            fclusters = [i for i, cls in enumerate(self.clusters) if set(cls) <= fatoms]  # fatoms is a set as well
...
if __name__ == "__main__":
    ...
    for s in test_smiles:
        ...
        hmol = MolGraph(s)
        print('fragments')
        print(MolGraph.FRAGMENTS)
        print('clusters')
        print(hmol.clusters)
        print(len(hmol.clusters))
        # print(list(hmol.mol_tree.edges))
        print('node attrs')
        print(nx.get_node_attributes(hmol.mol_tree, 'label'))
