...
import pickle
from data_utils import qm9, tu_mutag
...
class GNNBPETrainer:
    def __init__(self, cfg):
        ...
        print('======================train normal GNN with GraphBPE is called======================')
    ...
    def prepare_data(self, round_id):
        ...
        for data in self.dataset:
            mol_name = data.name
            y = data.y
            adj, node_names = tok_dataset[mol_name]['adj'], tok_dataset[mol_name]['node_names']
            adj = torch.from_numpy(adj)
            edge_index = adj.nonzero().t().contiguous()
            features = torch.from_numpy(tok_dataset[mol_name]['node_features']).to(dtype=torch.float)
            assert len(adj) == features.shape[0]
            processed_data.append(Data(x=features, edge_index=edge_index, y=y))
        num_workers = 0
        if self.k_fold:
...
def main(cfg: DictConfig):
    gnn_trainer = GNNBPETrainer(cfg=cfg)
