...
from utils.make_hyper_dataset import get_hyperedge_index
from utils.FG_BERT_find_fg import get_common_fg, split_node_w_fg
from tqdm import tqdm
from h2g import h2g_tok


class HGNNTrainer:
    """
    for baseline tokenization methods ('chem' (decomposition based on functional groups) & 'h2g' (for the paper: https://arxiv.org/pdf/2002.03230.pdf))
    """
    def __init__(self, cfg):
        ...
        self.construct_hyper_method = self.train_cfg.construct_hyper_method
        dataset_name2dir = {'qm9': 'qm9', 'mutag': 'MUTAG', 'enzymes': 'ENZYMES', 'proteins': 'PROTEINS',
                            'esol': 'esol', 'freesolv': 'freesolv', 'lipophilicity': 'lipo'}
        self.hypergraph_name2split_path = 'dataset/{}/{}.pickle'.format(dataset_name2dir[self.dataset_name], self.construct_hyper_method)

        if not os.path.exists(self.hypergraph_name2split_path):
            if self.construct_hyper_method == 'chem':
                print('==========================tokenizing dataset with functional groups==========================')
                name2split = {}
                common_fgs = get_common_fg()
                for data in tqdm(self.dataset):
                    smi = data.smiles
                    mol_name = data.name
                    name2split[mol_name] = split_node_w_fg(smiles=smi, num_atoms=data.x.shape[0], fg_structures=common_fgs)
                with open(self.hypergraph_name2split_path, 'wb') as handle:
                    pickle.dump(name2split, handle, protocol=pickle.HIGHEST_PROTOCOL)
            elif self.construct_hyper_method == 'h2g':
                print('==========================tokenizing dataset with hyper2hyper==========================')
                name2smi_dict = {}
                for data in tqdm(self.dataset):
                    smi = data.smiles
                    mol_name = data.name
                    name2smi_dict[mol_name] = smi
                name2split = h2g_tok.tokenize_dataset(name2smi_dict=name2smi_dict, ncpu=16, min_frequency=100)
                with open(self.hypergraph_name2split_path, 'wb') as handle:
                    pickle.dump(name2split, handle, protocol=pickle.HIGHEST_PROTOCOL)
            else:
                raise NotImplementedError

        print('======================train hyper GNN w/o tokenization is called======================')

    def prepare_data(self, round_id):
        with open(self.hypergraph_name2split_path, 'rb') as handle:
            name2split = pickle.load(handle)
        hyper_dataset = []
        for data in self.dataset:
            y = data.y
            mol_name = data.name
            node_split = name2split[mol_name]
            hyperedge_index = get_hyperedge_index(edge_index=data.edge_index.numpy(),
                                                  node_split=node_split)

            hyper_dataset.append(Data(x=data.x, hyperedge_index=hyperedge_index, y=y))
    ...
    def get_results(self):
        ...
        if self.k_fold:
            ...
            for i in range(self.k_fold):
                ...
                ori_trainer = PLTrainerWrapperForGNN(type_id='tok_{}'.format(self.construct_hyper_method),
        ...
        if self.dataset_cfg.num_class == 1:  # regression task
            ori_dataframe.to_csv(save_dir + '/tok_{}_RMSAE.csv'.format(self.construct_hyper_method))  # RMSE, MSE, MAE
        else:  # classification task
            ori_dataframe.to_csv(save_dir + '/tok_{}_acc.csv'.format(self.construct_hyper_method))
...
def main(cfg: DictConfig):
    hgnn_trainer = HGNNTrainer(cfg=cfg)
