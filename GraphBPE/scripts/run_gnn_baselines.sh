...
for model_type in "${MODELS[@]}"; do
    for hidden_channels in 32 64; do
        for num_layers in 1 2 3; do
            python train_GNN.py dataset="$1" model="$model_type" model.num_layer="$num_layers" model.hidden_channels="$hidden_channels" train.lr=1e-2 train.batch_size="$BATCH_SIZE"
            python train_GNN.py dataset="$1" model="$model_type" model.num_layer="$num_layers" model.hidden_channels="$hidden_channels" train.lr=1e-3 train.batch_size="$BATCH_SIZE"
        done
    done
done
