...
BATCH_SIZE=512
MODELS=(hyper_conv hgnn hgnnp hnhn)
    ...
    for hidden_channels in 32 64; do
        for num_layers in 1 2 3; do
            for lr in 1e-2 1e-3; do
                python train_HGNN.py dataset="$1" model="$model_type" model.num_layer="$num_layers" model.hidden_channels="$hidden_channels" train.lr="$lr" train.batch_size="$BATCH_SIZE" train.construct_hyper_method='h2g'
                python train_HGNN.py dataset="$1" model="$model_type" model.num_layer="$num_layers" model.hidden_channels="$hidden_channels" train.lr="$lr" train.batch_size="$BATCH_SIZE" train.construct_hyper_method='chem'
            done
        done
    done
done
